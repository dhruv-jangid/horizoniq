import type { MenuCheckboxItem } from "@base-ui/react/menu";
import {
  Add01Icon,
  CircleIcon,
  LayersIcon,
...
function GlobalMap({
  ...
  return (
    <LeafletMapContainer
      ...
      zoomControl={false}
      className={cn("z-50 size-full min-h-96 flex-1 rounded-4xl", className)}
      {...props}
    />
  );
}
...
interface MapLayerGroupOption
  extends Pick<React.ComponentProps<typeof MenuCheckboxItem>, "disabled"> {
  name: string;
}
...
function MapMarker({
  ...
  return (
    <LeafletMarker
      icon={L.divIcon({
        html: renderToString(<HugeiconsIcon icon={icon} strokeWidth={2} />),
        iconAnchor,
        ...(bgPos ? { bgPos } : {}),
...
function MapLocateControl({
  ...
  return (
    <MapControlContainer className={cn("right-2 top-2", className)}>
      <Button
        type="button"
        size="icon"
        ...
        className="border border-black/70 size-10"
        {...props}
      >
        {isLocating ? (
          <HugeiconsIcon icon={LoaderCircle} className="animate-spin" />
        ) : (
          <HugeiconsIcon icon={Navigation03Icon} strokeWidth={2} className="size-5" />
        )}
      </Button>
      {position && <MapMarker position={position} icon={Locate} />}
    </MapControlContainer>
  );
}
...
function MapSearchControl({ className, ...props }: PlaceAutocompleteProps) {
  return (
    <MapControlContainer className={cn("top-2 left-2 z-1001 w-60", className)}>
      ...
      createDrawTool={(L, map) =>
        new L.Draw.Marker(map, {
          icon: L.divIcon({
            ...
            iconAnchor: [12, 12],
            html: renderToString(<HugeiconsIcon icon={MapPinIcon} className="size-6" />),
          }),
          ...props,
        })
      }
...
function MapDrawPolygon({
  ...
  return (
    ...
    >
      <HugeiconsIcon icon={PentagonIcon} />
    </MapDrawShapeButton>
  );
}
...
function MapDrawActionButton<T extends EditToolbar.Edit | EditToolbar.Delete>({
  ...
  const hasFeatures = featureGroup?.getLayers().length ?? false;
